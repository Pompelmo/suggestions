...
def loading():
    ...
    corpus = gensim.corpora.MmCorpus('source/web_text_mm_filtered.mm')     # corpus for tfidf MemoryUsage(MU): 13Mib

    print datetime.now(), "loading model for tf-idf"
    tfidf = gensim.models.TfidfModel.load('source/web_text_tifidf_filtered.tfidf_model')  # tfidf model MU: 95Mib
    lsi = gensim.models.LsiModel.load('source/lsi_prova.lsi')
    lsi_index = gensim.similarities.Similarity.load('source/lsiSimilarity.index')
    # index = gensim.similarities.Similarity.load('source/tfidfSimilarity.index')     # tfidf similarity matrix MU:0
    tfidf_dict = gensim.corpora.Dictionary.load('source/web_text_dict_filtered.dict')  # dict for word <-> id MU:118Mib

    print datetime.now(), "loading dictionary for tf-idf"
    tfidf_dict_file = open('source/web_text_doc_filtered.pkl', 'r')
    ...
    db_mean_value = shelve.open('source/mean_dict_key_db.db')
    ...
    d2v_model = gensim.models.Doc2Vec.load('source/doc2vec_stem')     # doc 2 vector model. MU:241Mib
    ...
    w2v_model = gensim.models.Word2Vec.load('source/w2v_keywords_20151201')    # word 2 vec model, MU:678Mib
    ...
    return corpus, tfidf, lsi, lsi_index, tfidf_dict, tfidf_web, db_mean_value, ball_tree,\
